...
/**
 * A class which holds all of the different options that the user
 * can change and configure to their liking.
 * 
 * You can add an option here and it will automatically get saved when you start
 * the game, just make sure to make it configurable by adding to an options substate!
 * You can do so by checking out the `options` package.
 */
